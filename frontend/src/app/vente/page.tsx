...
function ContactForm() {
  ...
  const handleContact = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/v1/contact', { nom, email, sujet: 'Conseil vente', message: `${nom} (${email}) souhaite parler à un conseiller depuis la page Vente.` })
      setSent(true)
    } catch {
...
const BUDGETS_VENTE = [
  ...
  { label: 'Moins de 5 000 000', max: 5000000 },
  { label: 'Moins de 15 000 000', max: 15000000 },
  { label: 'Moins de 30 000 000', max: 30000000 },
  { label: 'Moins de 50 000 000', max: 50000000 },
  { label: 'Moins de 100 000 000', max: 100000000 },
]
...
function BienCard({ bien }: { bien: Bien }) {
  const prix = bien.prix.toLocaleString('fr-FR') + ' FCFA'
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-shadow">
      <Link href={`/biens/${bien.id}`} className="block">
        ...
        <div className="p-4">
          ...
          <p className="font-extrabold text-base" style={{ color: '#E05C52' }}>{prix}</p>
        </div>
      </Link>
    </div>
  )
...
export default function VentePage() {
  ...
  const params: Record<string, string | number> = { nature: 'vente', per_page: 50 }
  if (type && type !== 'Tous') params.type = type
  if (ville) params.ville = ville
  if (budgetI > 0) params.prix_max = BUDGETS_VENTE[budgetI].max

  const { data: page, isLoading } = useQuery({
    queryKey: ['biens-vente', type, ville, budgetI],
      ...
      <section className="bg-white border-b border-gray-100 py-10 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-3">
            ...
            <span>/</span>
            <span style={{ color: '#E05C52' }}>Vente</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2">
            Investissez dans{' '}
            <span style={{ color: '#E05C52' }}>l&apos;exception</span>
          </h1>
          <p className="text-gray-500 text-sm">
            Notre sélection de biens à acquérir, accompagnés par des experts juridiques et financiers.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 w-full flex-1">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Budget max</label>
              <select
                value={budgetI}
                onChange={(e) => setBudgetI(Number(e.target.value))}
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:border-[#E05C52] transition-colors bg-white"
              >
                {BUDGETS_VENTE.map((b, i) => <option key={i} value={i}>{b.label}</option>)}
              </select>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start gap-10 justify-between">
          <div className="text-white max-w-md">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              Notre équipe est là pour vous aider à chaque étape. Contactez KerConnect
              pour acquérir votre bien rapidement, en toute transparence.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              <a href="tel:+221710307054" className="flex items-center gap-2 text-white text-sm border border-white/20 px-4 py-2.5 rounded-xl hover:bg-white/10 transition-colors">
                📞 +221 71 030 70 54
              </a>
